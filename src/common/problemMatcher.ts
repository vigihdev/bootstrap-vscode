...
abstract class AbstractLineMatcher implements ILineMatcher {
	...
	public handle(lines: string[], start = 0): HandleResult {
	...
	private fillProperty(data: ProblemData, property: keyof ProblemData, pattern: ProblemPattern, matches: RegExpExecArray, trim = false): void {
	...
	private createProblemMatcher(description: Config.ProblemMatcher): ProblemMatcher | null {
		...
		if (Types.isString(description.base)) {
			...
			if (variableName.length > 1 && variableName[0] === '$') {
				const base = this.registry.get(variableName.substring(1));
				if (base) {
